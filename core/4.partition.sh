...
diskchoose() {
	...
	while true; do
		disk=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Exit to Menu" --menu "Disk/partition options" 0 0 0 \
		...
		0)
			...
			while true; do
				...
				"Format") while true; do
					...
					fsformat=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --stdout --menu "Please select the filesystem to be formated on $devdisk" 0 0 0 \
						...
						"$xtrafs" \
						"Swap" "Virtual memory partition" \
						"Unformated" "Empty/Wiped partition")
					[ $? = 0 ] || break
