...
"x86_64")is64=1;;"x86")is64=0;;*)dialog --backtitle "$BACKTITLE" --title "$TITLE" --msgbox "Your CPU is not supported, please install on another computer"
exit 1;;esac
...
keymapc(){
while true;do
KEYMAP=$(dialog --backtitle "$BACKTITLE" --title "Set the Keyboard Layout" --nocancel --stdout --default-item "us" --menu "Select a keymap that corresponds to your keyboard layout. Choose 'other' if your keymap is not listed. If you are unsure, the default is 'us'(United States/QWERTY).\n\nKeymap:" 0 0 0 "br-abnt2" "Brazilian Portuguese" "cf" "Canadian-French" "colemak" "Colemak(US)" "dvorak" "Dvorak(US)" "fr-latin1" "French" "de-latin1" "German" "gr" "Greek" "it" "Italian" "hu" "Hungarian" "jp" "Japanese" "pl" "Polish" "pt-latin9" "Portuguese" "ru4" "Russian" "es" "Spanish" "la-latin1" "Spanish Latinoamerican" "sv-latin1" "Swedish" "us" "United States" "uk" "United Kingdom" "other" "View all available keymaps")
[ "$KEYMAP" = "other" ]||break
keymaps=()
for map in $(localectl list-keymaps);do
keymaps+=("$map" "")
done
KEYMAP=$(dialog --backtitle "$BACKTITLE" --title "Set the Keyboard Layout" --cancel-label "Back" --stdout --menu "Select a keymap that corresponds to your keyboard layout. The default is 'us'(United States/QWERTY)." 0 0 0 "${keymaps[@]}")
[ $? = 0 ]&&break
done
...
localec(){
local title="Set the System Locale"
...
while true;do
LOCALE=$(dialog --backtitle "$BACKTITLE" --title "$title" --nocancel --stdout --default-item "en_US.UTF-8" --menu "$(menu_title "Choose 'other' if your language and/or region is not listed.") " 0 0 0 "en_AU.UTF-8" "English(Australia)" "en_CA.UTF-8" "English(Canada)" "en_US.UTF-8" "English(United States)" "en_GB.UTF-8" "English(Great Britain)" "fr_FR.UTF-8" "French(France)" "de_DE.UTF-8" "German(Germany)" "it_IT.UTF-8" "Italian(Italy)" "ja_JP.UTF-8" "Japanese(Japan)" "pt_BR.UTF-8" "Portuguese(Brazil)" "pt_PT.UTF-8" "Portuguese(Portugal)" "ru_RU.UTF-8" "Russian(Russia)" "es_MX.UTF-8" "Spanish(Mexico)" "es_ES.UTF-8" "Spanish(Spain)" "sv_SE.UTF-8" "Swedish(Sweden)" "vi_VN.UTF-8" "Vietnamese(Vietnam)" "zh_CN.UTF-8" "Chinese(Simplified)" "other" "View all available locales")
[ "$LOCALE" = "other" ]||break
locales=()
while read -r line;do
locales+=("$line" "")
done < <(grep -E "^#?[a-z].*UTF-8" /etc/locale.gen|sed -e 's/#//' -e 's/\s.*$//')
LOCALE=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --stdout --menu "$(menu_title)" 0 0 0 "${locales[@]}")
[ $? = 0 ]&&break
done
LOC_STAT="*"
}
...
regions+=("other" "")
while true;do
ZONE=$(dialog --backtitle "$BACKTITLE" --title "$title" --nocancel --stdout --menu "$(menu_title "If your region is not listed, select 'other'.\n")" 0 0 0 "${regions[@]}")
if [ "$ZONE" = "other" ];then
...
ZONE=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --stdout --menu "$(menu_title "")" 0 0 0 "${other_regions[@]}")
[ $? = 0 ]&&break
fi
zone_regions=()
...
SUBZONE=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --stdout --menu "$(menu_title "")" 0 0 0 "${zone_regions[@]}")
[ $? = 0 ]||continue
...
SUBZONE_SUBREGION=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --stdout --menu "$(menu_title "")" 0 0 0 "${subzone_regions[@]}")
if [ $? = 0 ];then
ZONE="${ZONE}/${SUBZONE}/${SUBZONE_SUBREGION}"
break
fi
done
dialog --backtitle "$BACKTITLE" --title "Set the Hardware Clock" --nocancel --stdout --yesno "Would you like to set the hardware clock from the system clock using UTC time?\nIf you select no, local time will be used instead.\n\nIf you are unsure, UTC time is the default." 0 0
[ $? = 0 ]||utc_enabled=false
TZC_STAT="*"
}
usrname(){
...
diskchoose(){
local diskconfirm=0
local title="Partition the harddrive"
listblk(){
opt=$1
shift
...
while true;do
disk=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Exit to Menu" --menu "Disk/partition options" 0 0 0 "Auto" "Automatically choosing disk and partition to install(alongside other operating systems)" "Basic" "Select disk/partition(s) to install" "Manual" "Customize disk/partition layout")
case "$disk" in
"Auto")for d in $(listblk NAME -d);do
...
done;;"Basic")simplediskman
[ "$diskconfirm" = 1 ]&&return;;"Manual")if dialog --backtitle "$BACKTITLE" --title "Manual partitioning" --yes-label "Use Terminal interface" --no-label "Use Command line interface" --yesno "Would you like to use the terminal interface or the command line interface?" 0 0;then
advcddiskman
[ "$diskconfirm" = 1 ]&&return
fi
clear
...
while true;do
if [ ${#devs[@]} = 0 ];then
dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "No device is available to install" 0 0
break
fi
devdisk=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --ok-label "Select" --stdout --menu "Select the disk/partition for ExtOS to be installed on. Note that the disk/partition you select will be erased, but not until you have confirmed the changes.\n\nSelect the disk in the list below:" 0 80 0 "${devs[@]}")
[ $? = 0 ]||break
...
flagasboot
fi;;"bios")flagasboot;;esac;;"disk")dialog --backtitle "$BACKTITLE" --title "Formatting $devdisk" --infobox "Creating GPT partition table on $devdisk" 0 0
...
lvmorcrypt=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "No" --stdout --menu "Do you want to use LVM and/or Encrypt the installation disk/partition?\n\nSelect the option in the list below:" 0 0 0 "No" "Do not use LVM or Encrypt the installation disk/partition" "Encrypt" "Encrypt the installation disk/partition" "$haslvm")
if [ $? = 0 ];then
case "$lvmorcrypt" in
...
devdisk=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --ok-label "Select" --extra-button --extra-label "Next" --stdout --menu "Select the disk/partition for ExtOS to be installed on. Note that the disk/partition you select will be erased, but not until you have confirmed the changes.\n\nSelect the disk in the list below:" 0 80 0 "${devs[@]}")
case $? in
0)devtype=$(lsblk -d -n -r -o TYPE "$devdisk")
...
mntopts=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --stdout --menu "${devtype^^} $devdisk Type: $devtype $dorpa Size: $(lsblk -d -n -r -o SIZE "$devdisk")\n In use: none\n\nChoose an action:" 0 0 0 "Mountpoint" "Use$dorpb as " "Format" "Format/Erase/Change filesystem of$dorpb" "$xtraopt" "$lvmopt")
[ $? = 0 ]||break
case "$mntopts" in
"Mountpoint")while true;do
mntpt=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --stdout --menu "Choose mountpoint for $devtype $devdisk:\n\nNote: everything except '/' and '/boot' are optional" 0 0 0 "/" "This is where the base system will be installed" "/boot" "Needed for UEFI/LVM(bios/mbr)/encryption" "/boot/efi" "(UEFI) EFI System partition" "/home" "Userspace data will be saved here(not apply for Frugal Installation)" "/usr" "App data will be stored here(not apply for Frugal Installation)" "/etc" "App Configurations will be stored here(not apply for Frugal Installation)" "/root" "Userspace data for root/admin will be stored here(not apply for Frugal Installation)" "/var" "Stores app data, must be mounted as read-write(not apply for Frugal Installation)" "/data" "(Frugal only) Data partition, the same as '/var' partition in normal installation" "/overlay" "(Frugal only) Overlay partition, for storing overlay data" "swap" "Virtual memory partition")
[ $? = 0 ]||break
if dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --yesno "Warning: All data on ${devtype^} $devdisk will be erased\n\nContinue?" 0 0;then
if say "${MNT_LST[@]}"|grep -wq "$mntpt";then
dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --yesno "Warning: The mountpoint $mntpt is already in use.\n\nContinue?" 0 0||break
...
xtrafs=
if [ "$(say "${MNT_LST[@]}"|grep "/boot"|awk '{print "$2"}')" ];then
xtrafs="\"Encrypted\" \"Encrypted filesystem, secure your data(/boot or /boot/efi is required)\""
fi
fsformat=$(dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --stdout --menu "Please select the filesystem to be formated on $devdisk" 0 0 0 "Ext2" "Standard Extended Filesystem for Linux version 2" "Ext3" "Ext2 with journaling" "Ext4" "Latest version of Extended Filesystem improved" "BTRFS" "Filesystem for storing and managing large volume of data provided by BtrFS" "XFS" "High-performance filesystem for server use" "JFS" "Journaled filesystem by IBM" "ZFS" "Filesystem for storing and managing large volume of data provided by OpenZFS" "FAT32" "Compatible, highly usable filesystem for storing data only(ExtOS Frugal installable)" "NTFS" "Standard Windows filesystem, use for data transfer only(ExtOS Frugal installable)" "F2FS" "Fast filesystem for storing data only(ExtOS Frugal installable)" "LVM" "Logical Volume Manager, useful if you want to have more partitions on disk that has 'msdos' partition table or when you have multiple disks(w or w/o RAID)" "$xtrafs" "Swap" "Virtual memory partition" "Unformated" "Empty/Wiped partition")
[ $? = 0 ]||break
dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --yesno "Warning: All data on ${devtype^} $devdisk will be erased\n\nContinue?" 0 0||break
...
case "$fsformat" in
Ext2|Ext3|Ext4)mkfs."${fsformat,,}" -F "$devdisk";;"XFS")mkfs.xfs -f "$devdisk";;"BTRFS"|"JFS"|"NTFS"|"F2FS")m kfs."${fsformat,,}" "$devdisk";;"ZFS")zfs_id=$(randstr)
zpool create -f "$zfs_id" "$devdisk";;"FAT32")mkfs.vfat -F 32 "$devdisk";;"LVM")pvcreate "$devdisk";;"Encrypted")while true;do
ecryptpass=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --inputbox "Please enter the password for the encrypted filesystem" 0 0)
[ $? = 0 ]||break
if [ ! "$ecryptpass" ];then
dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --msgbox "Password cannot be empty" 0 0
continue
fi
ecryptpass2=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --inputbox "Please re-enter the password to confirm" 0 0)
[ $? = 0 ]||break
if [ ! "$ecryptpass2" ];then
dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --msgbox "Password cannot be empty" 0 0
continue
...
dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --msgbox "Password does not match, please try again" 0 0
continue
fi
saybr "$ecryptpass\n$ecryptpass"|cryptsetup luksFormat "$devdisk"
saybr "$ecryptpass"|cryptsetup open "$devdisk" "$(randstr)"
done;;"Swap")mkswap "$devdisk";;"Unformated")wipefs -a "$devdisk";;esac
[ $? = 0 ]||dialog --backtitle "$BACKTITLE" --title "$title" --cancel-label "Back" --msgbox "Error while formating the partition $devdisk as $fsformat, please try again" 0 0
break
done;;"Decrypt")while true;do
cryptpass=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --inputbox "$devdisk appears to be an encrypted partition\nIt must be unlocked in order to continue\n\nPlease enter the encryption passphrase:" 0 0)
[ $? = 0 ]||break
[ "$cryptpass" ]||dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "ERROR: You didn't entered the encryption passphrase!"
say "$cryptpass"|cryptsetup open "$devdisk" "$(randstr)"&&break
dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "ERROR: Could not unlock the partition.\n\nPlease check the passphrase and try again."
done;;"Manage")cfdisk "$devdisk";;"Manage LVM")while true;do
vglist=("")
...
vgselect=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --extra-button --extra-label "Create" --menu "Select the volume group to manage\n\n Name Size Free UUID" 0 80 0 "${vglist[@]}")
case $? in
0)while true;do
pvofvg=($(vgs --noheadings -o pv_name "$vgselect"))
pvofvg_txt=
for pv in "${pvofvg[@]}";do pvofvg_txt="$pvofvg_txt $pv,";done
lvofvg=($(vgs --noheadings -o lv_name "$vgselect"))
lvofvg_txt=
for lv in "${lvofvg[@]}";do lvofvg_txt="$lvofvg_txt $lv,";done
vgoption=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --menu "Volume Group Infomation:\n\nVolume Group Name: $vgselect Size: $(vgs --noheadings -o vg_size "$vgselect")\nFree: $(vgs --noheadings -o vg_free "$vgselect")\nUUID: $(vgs --noheadings -o vg_uuid "$vgselect")\nPhysical Volumes: ${pvofvg_txt%,}\nLogical Volumes: ${lvofvg_txt%,}\n\nSelect an option:" 0 80 0 "Manage PV" "Manage Physical Volume attached to this Volume Group" "Manage LV" "Manage Logical Volume on this Volume Group" "Rename" "Rename this Volume Group" "Remove" "Remove this Volume Group")
[ $? = 0 ]||break
case "$vgoption" in
...
pvselect=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --extra-button --extra-label "Add" --ok-label "Select" --menu "Select the physical volume to manage\n\n Name Size Free UUID" 0 80 0 "${pvlist[@]}")
case $? in
0)while true;do
vg_grs=$(pvs --noheadings -o vg_name "$pvselect")
vg_grs_txt=
for vg in "${vg_grs[@]}";do vg_grs_txt="$vg_grs_txt $lv,";done
pvopts=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --menu "Physical Volume Infomation:\n\nPhysical Volume Name:$pvselect\nSize: $(pvs --noheadings -o pv_size "$pvselect")\nFree: $(pvs --noheadings -o pv_free "$pvselect")\nUUID: $(pvs --noheadings -o pv_uuid "$pvselect")\nVolume Group: ${vg_grs_txt%,}\n\nSelect an option:" 0 80 0 "Replace" "Replace this Physical Volume with a new one" "Remove from VG" "Remove this Physical Volume from this Volume Group" "Remove" "Completely remove this Physical Volume")
[ $? = 0 ]||break
case "$pvopts" in
"Replace")while true;do
listpvfree
pvnew=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --menu "Select the new Physical Volume to replace this one\n\n Name Size Free UUID" 0 80 0 "${pvfreelist[@]}")
[ $? = 0 ]||break
vgchange -a n "$vgselect"
pvmove "$pvselect" "$pvnew"
vgchange -a y "$vgselect"
break
done;;"Remove from VG")vgchange -a n "$vgselect"
pvmove "$pvselect"
vgreduce "$vgselect" "$pvselect"
vgchange -a y "$vgselect"
break;;"Remove")vgchange -a n "$vgselect"
pvmove "$pvselect"
vgreduce "$vgselect" "$pvselect"
pvremove "$pvselect"
vgchange -a y "$vgselect"
break;;esac
done;;1)break;;3)listpvfree
pvselect=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --ok-label "Select" --checklist "Select the physical volume to add to this Volume Group\n\n Name Size Free UUID" 0 80 0 "${pvfreelist[@]}")
[ $? = 0 ]||continue
vgextend "$vgselect" "$pvselect"||dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "ERROR: Could not add the physical volume to the volume group.\n\nPlease check the volume group and try again.";;esac
done;;"Manage LV")while true;do
lvlist=("")
for lv in "${lvofvg[@]}";do
lvinfo="$(lvs --noheadings -o lv_size,lv_free,lv_uuid "$lv"|awk '{for(i=1;i<NF;i++) printf "$i" " \t";print $NF}')"
lvlist+=("$lv" "$lvinfo")
done
lvselect=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --extra-button --extra-label "Remove" --ok-label "Rename" --menu "Select the physical volume to manage\n\n Name Size Free UUID" 0 80 0 "${lvlist[@]}")
case $? in
0)while true;do
newlvname=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --inputbox "Enter the new name for the logical partition" 0 0)
[ $? = 0 ]||break
if [ -z "$newlvname" ];then
dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "ERROR: You didn't entered the new name!"
continue
fi
if lvs --noheadings -o lv_name|grep -q "$newlvname";then
dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "ERROR: The logical partition $newlvname already exists!"
continue
fi
lvrename "$lvselect" "$newlvname"
done;;3)lvremove "$vgselect"/"$lvselect";;esac
break
done;;"Rename")while true;do
newvgname=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --inputbox "Enter the new name for the volume group" 0 0)
[ $? = 0 ]||break
if [ -z "$newvgname" ];then
dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "ERROR: You didn't entered the new name!"
continue
...
dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "ERROR: The volume group $newvgname already exists!"
continue
fi
vgchange -a n "$vgselect"
...
break
done;;"Remove")vgchange -an "$vgselect"
vgremove "$vgselect"
break;;esac
break
done;;1)break;;3)listpvfree
pvselect=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --cancel-label "Back" --ok-label "Create" --checklist "Select the physical volumes to add to the volume group\n\n Name Size Free UUID" 0 80 0 "${pvfreelist[@]}")
[ $? = 0 ]||continue
newvgname=$(dialog --backtitle "$BACKTITLE" --title "$title" --stdout --inputbox "Please enter the name of the new volume group" 0 0)
[ $? = 0 ]||continue
if [ ! "$newvgname" ];then
dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "ERROR: You didn't entered the name of the new volume group!"
continue
fi
...
dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "Successfully created the volume group $newvgname"
else
dialog --backtitle "$BACKTITLE" --title "$title" --msgbox "ERROR: Could not create the volume group $newvgname!"
fi;;esac
done;;esac
break
done;;3)diskconfirm=1
break;;*)break;;esac
done
}
ossel(){
while true;do
os_base=$(dialog --backtitle "$BACKTITLE" --title "$TITLE" --stdout --cancel-label "Exit to menu" --menu "Choose based distro" 0 0 0 "Arch" "Arch Linux based full installation" "Debian" "Debian based full installation" "Frugal" "Minimal frugal installation" "Frugal Extended" "ExtCore full installation")
if [ $? != 0 ];then
menusel
return
fi
...
initsel(){
while true;do
initype=$(dialog --backtitle "$BACKTITLE" --title "$TITLE" --stdout --cancel-label "Back" --radiolist "Use arrow keys and Space to select which init system you would prefered\n\nComparision: https://wiki.gentoo.org/wiki/Comparison_of_init_systems" 0 0 0 "SystemD" "Standard init system, provides more than just an init system" on "OpenRC" "SysVinit based, suitable for minimal installation" off "runit" "A daemontools-inspired process supervision suite" off)
[ $? = 0 ]||break
case "$initype" in
...
libcsel(){
while true;do
libctype=$(dialog --backtitle "$BACKTITLE" --title "$TITLE" --stdout --cancel-label "Back" --radiolist "Use arrow keys and Space to select which libc you would prefered\n\nComparision: https://wiki.gentoo.org/wiki/Comparison_of_libc" 0 0 0 "glibc" "Standard libc, supports most of the modern linux kernel" on "musl" "Minimal libc, supports only the bare minimum for the kernel" off)
[ $? = 0 ]||break
pisel
[ "$OS_STAT" ]&&return
done
